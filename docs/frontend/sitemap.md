...
This code is used to dynamically generate a sitemap using data fetched from an API or database via the `fetchSitemap` hook. **To use this, you must create menu items on your cockpit CMS.**
...
export default async function sitemap() {
    ...
    const listmain = data.map(item => ({
        url: process.env.NEXT_PUBLIC_UR_DOMAIN + item.routes.default,  // Construct the URL based on the route
...
-   **`listmain`**: An array generated by mapping each item from the data to an object containing the URL and the last modification time (`lastModified`).
    -   **`url`**: Constructed from the domain, retrieved from the environment variable (`process.env.NEXT_PUBLIC_UR_DOMAIN`), and appended with the route from the data.
...
-   **`process.env.NEXT_PUBLIC_UR_DOMAIN`**: This environment variable must be set to identify the main domain of the site.